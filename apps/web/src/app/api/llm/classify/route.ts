...
function buildClassifyMessages(
  ...
  return [
    ...
    {
      role: "user",
      content: `Clasifica este código de algoritmo y responde únicamente en JSON con el formato {"kind":"iterative|recursive|hybrid|unknown"}. Código:\n\n${source}`,
    },
  ];
}

function parseKindFromGemini(
  data: GeminiResponse,
): ClassifyResponse["kind"] | null {
...
async function classifyWithLLM(
  ...
  if (kind) return kind;
  console.warn(
    `[Classify API] Fallback a backend Python: LLM response no válida`,
  );
      ...
      if (mode === "llm") {
        ...
        if (geminiApiKey) {
          try {
            ...
            console.warn(
              `[Classify API] LLM falló, usando backend Python: ${error instanceof Error ? error.message : String(error)}`,
            );
