...
export default function GrammarDebugPage() {
  ...
  useEffect(() => {
    try { setLocalOk(parseExpr(input)); }
    catch (e: any) { setLocalOk(false); setErr(String(e?.message || e)); }
  }, [input]);

  async function callApi() {
    setErr(null); setApiOk(null); setApiAvail(null);
    ...
    try {
      ...
      if (isGrammarParseResponse(data)) {
        setApiOk(data.ok); setApiAvail(data.available);
  ...
  return (
    <main style={{ padding: 24 }}>
      ...
      <input
        ...
        style={{ border: "1px solid #ddd", borderRadius: 6, padding: "8px 10px", width: 420, marginBottom: 12, color: "black" }}
        placeholder="(1+2)*3"
      />
      ...
      <button
        onClick={callApi}
        style={{ background: "#2563eb", color: "white", padding: "8px 12px", borderRadius: 6, border: 0 }}
      >
      ...
      <div style={{ marginTop: 12 }}>
        ...
        {apiAvail === false && <span style={{ marginLeft: 8, color: "#fbbf24" }}>(aa_grammar no disponible)</span>}
      </div>
