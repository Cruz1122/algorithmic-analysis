...
export default function RecursionTreeModal({
  ...
  onClose,
  recurrence,
  recursionTreeData: _recursionTreeData,
  characteristicEquation,
    ...
    try {
      ...
      if (isLinearRecurrence) {
        ...
        if (recurrence.b <= 1 || recurrence.a <= 0) {
          console.warn(
            "[RecursionTreeModal] Invalid recurrence parameters for divide-and-conquer tree:",
            recurrence,
          );
          setNodes([]);
          setEdges([]);
  ...
  if (isLinearRecurrence) {
    ...
    if (
      !recurrence.shifts ||
      recurrence.shifts.length === 0 ||
      !recurrence.coefficients ||
      recurrence.coefficients.length === 0
    ) {
      return null;
    }
  } else if (isDivideConquer) {
