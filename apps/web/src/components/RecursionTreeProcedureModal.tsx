        ...
        <div className="flex-1 overflow-y-auto p-4 scrollbar-custom">
          <div className="grid grid-cols-1 lg:grid-cols-12 gap-4 h-full">
            ...
            <div className="space-y-4 lg:col-span-7 flex flex-col min-h-0">
              ...
              {recurrence && (
                <div className="p-3 rounded-lg bg-slate-800/50 border border-white/10 flex-shrink-0">
                  <div className="flex items-center justify-between mb-2">
                    <h4 className="text-white font-semibold text-sm">Ecuación de Recurrencia</h4>
              ...
              {/* Tabla por Niveles */}
              {recursionTree && recursionTree.table_by_levels && recursionTree.table_by_levels.length > 0 && (
                <div className="p-3 rounded-lg bg-slate-800/50 border border-white/10 flex flex-col flex-1 min-h-0">
                  <h4 className="text-white font-semibold mb-3 text-sm flex-shrink-0">Tabla por Niveles</h4>
                  <div className="overflow-auto flex-1 scrollbar-custom">
                    <table className="w-full text-xs border-collapse">
                      <thead className="sticky top-0 bg-slate-800/95 z-10">
                        <tr className="border-b border-white/20">
                          <th className="text-left p-1.5 text-slate-300 font-semibold">i</th>
                          <th className="text-left p-1.5 text-slate-300 font-semibold"># Nodos</th>
                          <th className="text-left p-1.5 text-slate-300 font-semibold">Tamaño</th>
                          <th className="text-left p-1.5 text-slate-300 font-semibold">Costo/nodo</th>
                          <th className="text-left p-1.5 text-slate-300 font-semibold">Costo total</th>
                        </tr>
                      </thead>
                      <tbody>
                        {recursionTree.table_by_levels.map((row, idx) => (
                          <tr key={idx} className="border-b border-white/10 hover:bg-slate-700/30 transition-colors">
                            <td className="p-1.5 text-white font-medium text-center">{row.level}</td>
            ...
            <div className="space-y-3 lg:col-span-5">
              ...
              {recursionTree && recursionTree.height && (
                <div className="p-3 rounded-lg bg-slate-800/50 border border-white/10">
                  <h4 className="text-white font-semibold mb-2 text-sm">Altura</h4>
              ...
              {recursionTree && recursionTree.summation && (
                <div className="p-3 rounded-lg bg-slate-800/50 border border-white/10">
                  <h4 className="text-white font-semibold mb-2 text-sm">Sumatoria</h4>
                  <div className="bg-slate-900/50 p-2 rounded border border-white/10 overflow-x-auto">
                    ...
                    <div className="text-xs text-slate-400 mb-1 mt-2">Evaluada:</div>
                    <div className="flex justify-center scale-90 origin-center">
              ...
              {recursionTree && recursionTree.dominating_level && (
                <div className="p-3 rounded-lg bg-slate-800/50 border border-white/10">
                  <h4 className="text-white font-semibold mb-2 text-sm">Nivel Dominante</h4>
                  <div className="bg-slate-900/50 p-2 rounded border border-white/10 overflow-x-auto">
                    <div className="flex justify-center scale-90 origin-center">
