...
const getSectionIcon = (sectionId: string, size: number = 16) => {
  ...
  switch (sectionId) {
    case "monorepo-packages":
      return <Package {...iconProps} className={`${iconProps.className} text-purple-400`} />;
    case "code-quality":
...
const getCategoryInfo = (section: DocumentationSection) => {
  if (section.content?.type === "packages") return { category: "Arquitectura", priority: 1 };
  if (section.content?.type === "tools") return { category: "Herramientas", priority: 2 };
  if (section.content?.type === "katex") return { category: "Componentes", priority: 3 };
  if (section.content?.type === "analyzer") return { category: "Interfaces", priority: 4 };
  if (section.id === "ui-showcase") return { category: "Demostración", priority: 5 };
  return { category: "Documentación", priority: 0 };
};

export const DocumentationIndex = ({ sections, onSectionClick }: DocumentationIndexProps) => {
  ...
  const sortedCategories = Object.entries(groupedSections).sort(([catA, sectionsA], [catB, sectionsB]) => {
  ...
  return (
    <div className="documentation-index glass-card p-4 rounded-lg border border-white/10 mb-6">
      <div className="flex items-center justify-between mb-4">
        <h3 className="text-lg font-semibold text-white">Índice de Contenidos</h3>
        <button
        ...
        {sortedCategories.map(([category, categorySections]) => (
          <div key={category}>
            ...
            >
              ...
              <span>{category}</span>
              <span className="text-xs text-slate-500">({categorySections.length})</span>
            </button>
      ...
      <div className="mt-6 pt-4 border-t border-white/10">
        <div className="grid grid-cols-2 gap-4 text-center">
          <div>
            <div className="text-lg font-bold text-white">{sections.length}</div>
            <div className="text-xs text-slate-400">Secciones</div>
          </div>
          <div>
            <div className="text-lg font-bold text-white">{sortedCategories.length}</div>
