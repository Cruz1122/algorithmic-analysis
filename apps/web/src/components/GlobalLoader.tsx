...
const SpinnerLoader: React.FC<{ size: LoaderSize }> = ({ size }) => (
  <div className={`${sizeConfig[size].spinner} animate-spin rounded-full border-2 border-slate-300/30 border-t-blue-500`} />
);
...
const ProgressLoader: React.FC<{ progress: number; size: LoaderSize }> = ({ progress, size }) => {
  ...
  return (
    <div className={`${width} ${height} bg-slate-700/50 rounded-full overflow-hidden backdrop-blur-sm`}>
      <div
...
const PulseLoader: React.FC<{ size: LoaderSize }> = ({ size }) => (
  <div className="flex items-center justify-center">
    ...
    <div className={`${sizeConfig[size].spinner} bg-blue-500/40 rounded-full animate-ping absolute`} style={{ animationDelay: '0.5s' }} />
...
export const GlobalLoader: React.FC<GlobalLoaderProps> = ({
  ...
  return (
    <div className={`flex flex-col items-center justify-center gap-3 ${className}`}>
      {renderLoader()}
      {message && (
        <p className={`${sizeConfig[size].text} text-slate-300 font-medium text-center max-w-xs animate-pulse`}>
          {message}
        </p>
      )}
      {variant === 'progress' && (
