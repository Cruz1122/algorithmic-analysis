...
export function ASTTreeView({ node, level = 0 }: ASTTreeViewProps) {
  ...
  const nodeType = 'type' in node && typeof node.type === 'string' ? node.type : 'Object';
  ...
  return (
    <div className={level > 0 ? 'ml-4' : ''}>
      ...
      >
        <span className={`material-symbols-outlined text-sm text-yellow-400 transition-transform ${isExpanded ? 'rotate-90' : ''}`}>
          chevron_right
        </span>
